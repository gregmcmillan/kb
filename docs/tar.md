...
Typically, make the filename that you want to save end with ``.tgz``. This means it's a tar file that has been gzipped (compressed).
...
Step 1. Use ``tar cvf`` a directory then gzip it:
...
Step 2. Use ``gunzip`` then ``tar xvf``:
