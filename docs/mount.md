...
Here is an example using ``mount`` and ``fstab``.
...
1) At the top level, create a mount point, such as ``mkdir dos``. Call it whatever I want to:
...
3) (optional) Put a commented out entry in the fstab to remind myself of the slice name or file type that I use to mount.
...
5) Verify that the new slice is mounted and try accessing the slice:
